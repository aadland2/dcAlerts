...
import re
import sys 
sys.path.append('C:\\Users\\aadlanma\\Desktop\\geocodingApp')
import commonRegex
import os 
os.getcwd()
...
def regexWrapper(i):
    if numeric_extract(i):
        return numeric_extract(i)
    elif intersection_extract(intersection_string,i):
        return  intersection_extract(intersection_string,i)
      
    elif alpha_extract(i):
        return alpha_extract(i)
    elif block_extract(i):
        return  block_extract(i).strip()
    else:
        commoni = commonRegex.CommonRegex(i)
        if len(commoni.street_addresses) > 0:
            return commoni.street_addresses[0]
        else:
